...
exports.setorder = (req,res)=>{
	resobj = {};
	//判断是否登录，如果没有登录，直接返回错误信息
	if(req.session.site_user==null){
		resobj.status = NOLOGIN;
		resobj.message = "登录之后才能下单";
	...
	let orderSql = `
				...
				(				 
				 ...
				 ,${req.session.site_user.uid} /* user_id - INT(10)*/
				 ,'${req.session.site_user.uname}' /* user_name - VARCHAR(100)*/
...
exports.paysuccess= (req,res)=>{
	...
	let sql = `update dt_orders set status = ${payed},payment_time = NOW() where order_no="${orderno}"`;
